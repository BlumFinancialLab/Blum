...
def canonical_evidence_class(value: str | None, trading_mode: str | None = None) -> str:
...
def evaluate_copy_readiness(context: ReadinessContext, thresholds: ReadinessThresholds) -> ReadinessDecision:
    ...
    forward_sample = max(0, _integer(context.forward_sample))

    if forward_sample == 0:
        status = "REPLAY_ONLY" if _integer(context.replay_sample) > 0 else "NOT_READY"
...
def _paper_gate_results(context: ReadinessContext, thresholds: ReadinessThresholds) -> tuple[tuple[str, bool], ...]:
    ...
    return (
        ("global_forward_trades", _integer(context.global_forward_sample) >= thresholds.global_forward_trades),
        ("strategy_forward_trades", _integer(context.forward_sample) >= thresholds.strategy_forward_trades),
...
def _high_confidence_ready(context: ReadinessContext, thresholds: ReadinessThresholds) -> bool:
    return (
        _integer(context.global_forward_sample) >= thresholds.high_confidence_global_forward_trades
        and _integer(context.forward_sample) >= thresholds.high_confidence_strategy_forward_trades
...
def _capital_eligible(context: ReadinessContext, thresholds: ReadinessThresholds) -> bool:
    return (
        _integer(context.global_forward_sample) >= thresholds.capital_global_forward_trades
        and _integer(context.forward_sample) >= thresholds.capital_strategy_forward_trades
